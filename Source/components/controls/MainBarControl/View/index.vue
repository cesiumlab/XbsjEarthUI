        ...
        <div class="xbsj-list" ref="container" @mousedown="startMove($event)" @mousemove="onMoving($event)" @mouseup="endMove($event)">
            <div class="xbsj-list-item">
                ...
                <div class="xbsj-item-btnbox" @click="scalecontrolShow=!scalecontrolShow">
                    <div class="xbsj-item-btn scalebutton" :class="scalecontrolShow ? 'scalebuttonActive' : ''"></div>
                    <span class="xbsj-item-name">{{lang.scalecontrol}}</span>
                </div>
                <div class="xbsj-item-btnbox" @click="statecontrolShow=!statecontrolShow">
                    <div class="xbsj-item-btn statebutton" :class="statecontrolShow ? 'statebuttonActive' : ''"></div>
                    <span class="xbsj-item-name">{{lang.statecontrol}}</span>
                </div>
                <div class="xbsj-item-btnbox" @click="timelinecontrolShow=!timelinecontrolShow">
                    <div class="xbsj-item-btn timebutton" :class="timelinecontrolShow ? 'timebuttonActive' : ''"></div>
        ...
        <div class="xbsj-item-btnbox">
          ...
          <span class="xbsj-item-name">{{lang.timeaxis}}</span>
            ...
            <div class="xbsj-list-item">
                ...
                <div class="xbsj-item-btnbox ml20" @click="popup=!popup" v-show="(splitX > 0 && splitX < 1 && !(splitY > 0 && splitY < 1))">
                    <div
                ...
                <div class="xbsj-item-btnbox" @click="popup=!popup" v-show="(!(splitX > 0 && splitX < 1) && splitY > 0 && splitY < 1)">
                    <div
            ...
            <div class="xbsj-list-item xbsj-list-lastitem">
                ...
                <div class="xbsj-item-btnbox ml20" @click="terrainDebug=!terrainDebug">
                    <div class="xbsj-item-btn terrainbutton" :class="terrainDebug ? 'terrainbuttonActive' : ''"></div>
...
export default {
	...
	mounted() {
		...
		this.$nextTick(() => {
			this._unbinds.push(XE.MVVM.bind(this, 'navcontrolShow', this.$root.$earth.camera.navigator, 'showCompass'))
			this._unbinds.push(XE.MVVM.bind(this, 'scalecontrolShow', this.$root.$earth.camera.navigator, 'showDistanceLegend'))
			this._unbinds.push(XE.MVVM.bind(this, 'splitX', this.$root.$earth, 'camera.viewport.splitX'))
			this._unbinds.push(XE.MVVM.bind(this, 'splitY', this.$root.$earth, 'camera.viewport.splitY'))
			this._unbinds.push(XE.MVVM.bind(this, 'terrainDebug', this.$root.$earth, 'misc.debug.cesiumInspectorVisibility'))
			this._unbinds.push(XE.MVVM.bind(this, 'cesiumDebug', this.$root.$earth, 'misc.debug.tilesetInspectorVisibility'))

			this._unbinds.push(XE.MVVM.bind(this, 'baseColor', this.$root.$earth.terrainEffect, 'baseColor'))
		})
	},
	beforeDestroy() {
	...
	methods: {
		...
		startMove(event) {
			//如果事件的目标不是本el 返回
			if (event.target.parentElement !== this.$refs.container && event.target.parentElement.parentElement !== this.$refs.container) {
				this.moving = false
				return
			}
			this.moving = true
