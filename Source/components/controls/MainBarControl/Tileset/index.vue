    ...
    <div class="xbsj-list" ref="container" @mousedown="startMove($event)" @mousemove="onMoving($event)"
      ...
      <div class="xbsj-list-item">
        ...
        <div class="xbsj-item-btnbox ml20">
          <div class="xbsj-item-btn onlinebutton" @click="modelOnline=!modelOnline" :class="{highlight:modelOnline}">
          </div>
          <span class="xbsj-item-name">{{lang.online}}</span>
        </div>
        <div class="xbsj-item-btnbox" v-show="labServiceUI">
          <div class="xbsj-item-btn localhostbutton" @click="modelLab=!modelLab" :class="{highlight:modelLab}"></div>
          <span class="xbsj-item-name">{{lang.localhost}}</span>
        </div>
        <div class="xbsj-item-btnbox" v-show="cloudServiceUI">
          <div class="xbsj-item-btn localhostbutton" @click="modelCloud=!modelCloud" :class="{highlight:modelCloud}">
          </div>
        ...
        <div class="xbsj-item-btnbox">
          <div class="xbsj-item-btn ">
            ...
            <button class="localbutton" :disabled="!isLocalhost" @click="openLocal">
            </button>
          </div>
          <span class="xbsj-item-name">{{lang.localUpload}}</span>
        </div>
      </div>
      ...
      <div class="xbsj-list-item">
        ...
        <div class="xbsj-item-btnbox">
          <div class="xbsj-item-btn">
            <button class="movebutton" :class="{highlight:positionEditing}" :disabled="!enabled"
              @click="toggleMove"></button>
          </div>
        ...
        <div class="xbsj-item-btnbox">
          <div class="xbsj-item-btn">
            <button class="rotatebutton" :class="{highlight:rotationEditing}" :disabled="!enabled"
              @click="toggleRotate"></button>
          </div>
        ...
        <div class="xbsj-item-btnbox">
          <div class="xbsj-item-btn">
            <button class="lefttopButton" :class="xbsjLeftTopView ? 'lefttopButtonActive' : ''"
              @click="xbsjLeftTopView = !xbsjLeftTopView;" :disabled="!enabled"></button>
            <button class="righttopButton" :class="xbsjRightTopView ? 'righttopButtonActive' : ''"
              @click="xbsjRightTopView = !xbsjRightTopView" :disabled="!enabled"></button>
            <button class="leftbottomButton" :class="xbsjLeftBottomView ? 'leftbottomButtonActive' : ''"
              @click="xbsjLeftBottomView=!xbsjLeftBottomView" :disabled="!enabled"></button>
            <button class="rightbottomButton" :class="xbsjRightBottomView ? 'rightbottomButtonActive' : ''"
              @click="xbsjRightBottomView=!xbsjRightBottomView" :disabled="!enabled"></button>
          </div>
          <span class="xbsj-item-name">{{lang.view}}</span>
        </div>
        <div class="xbsj-item-btnbox">
          <div class="xbsj-item-btn">
            <button class="techonlogybutton" :class="{highlight:technologyShader}" :disabled="!enabled"
              @click="toggleTechnology"></button>
          </div>
          <span class="xbsj-item-name">{{lang.techonlogy}}</span>
        </div>
        <div class="xbsj-item-btnbox">
          <div class="xbsj-item-btn">
            <button class="doubleSidebutton" :class="{highlight:doubleSideProp}" :disabled="!enabled"
              @click="toggleDoubleSide"></button>
          </div>
        ...
        <div class="xbsj-item-btnbox">
          <div class="xbsj-item-btn">
            <button class="skipLevelOfDetailbutton" :class="skipLevelOfDetail ? 'skipLevelOfDetailActive' : ''"
              @click="skipLevelOfDetail =! skipLevelOfDetail" :disabled="!enabled"></button>
          </div>
        ...
        <div class="xbsj-item-btnbox">
          <div class="xbsj-item-btn">
            <button class="debugShowBoundingVolumebutton"
              :class="debugShowBoundingVolume ? 'debugShowBoundingActive' :''"
              @click="debugShowBoundingVolume = !debugShowBoundingVolume" :disabled="!enabled"></button>
          </div>
      ...
      <div class="xbsj-list-item">
        ...
        <div class="xbsj-slide-group">
          ...
          <div class="xbsj-slide-top">
            ...
            <div class="xbsj-slide-div">
              <XbsjSlider :min="-4" :max="8" :step="0.1" v-model="ssePower" :disabled="!enabled" :show-tip="showTip">
              </XbsjSlider>
            </div>
          ...
          <div class="xbsj-slide-bottom">
            ...
            <div class="xbsj-slide-div">
              <XbsjSlider :min="0" :max="5.0" :step="0.02" v-model="luminanceAtZenith" :disabled="!enabled"
                :show-tip="showTip"></XbsjSlider>
            </div>
        ...
        <div class="xbsj-slide-group">
          ...
          <div class="xbsj-slide-top">
            ...
            <div class="xbsj-slide-div">
              <XbsjSlider :min="0" :max="1.0" :step="0.01" v-model="imageBasedLightingFactor[0]" :disabled="!enabled"
                :show-tip="showTip"></XbsjSlider>
            </div>
          ...
          <div class="xbsj-slide-bottom">
            ...
            <div class="xbsj-slide-div">
              <XbsjSlider :min="512" :max="5120" :step="1" v-model="maximumMemoryUsage" :disabled="!enabled"
                :show-tip="showTip"></XbsjSlider>
            </div>
  ...
  export default {
    ...
    methods: {
      ...
      openLocal() {
        ...
        // let filepath = "D:\\测试数据\\AB_测试数据合集\\通用模型处理\\SHP\\新建文件夹\\tileset.json"
        //http://localhost:9003
        let url = "/filedialog"
        axios.post(url, data).then(res => {
          if (res.data && res.data.data && res.data.data.path) {
            var path = res.data.data.path[0]
            if (path != 'cancelled') {
      ...
      uploadServe(params) { //发布服务
        ...
        axios.post(url, params).then(res => {
          ...
          if (res && res.data.data.status) {
            let options = {
              name: params.name,
              url: 'http://localhost:9003/model/' + res.data.data.id + '/tileset.json'
            }
            console.log("服务发布成功", options)
            this.addSenceTree(options)
          }
      ...
      addSenceTree(options) { //添加到场景树
        const earth = this.$root.$earthUI.earth
        const g0 = new XE.SceneTree.Group(earth) //全局场景g0
        earth.sceneTree.root.children.push(g0)
        const tileset = new XE.Obj.Tileset(earth)
        tileset.name = options.name
        tileset.url = options.url
        g0.children.push(new XE.SceneTree.Leaf(tileset))
        //监控 ready
        var unbind = XE.MVVM.watch(() => {
...
<style scoped>
  ...
  .debugShowBoundingVolumebutton:disabled {
    background: url(../../../../images/debugShowBoundingVolume_disabled.png) no-repeat;
    background-size: contain;
    cursor: not-allowed;
  }
