...
function destroyVueNode(ivuNode, parent) {
	if (ivuNode._inner.disposer) {
		ivuNode._inner.disposer.basicPropDisposer && ivuNode._inner.disposer.basicPropDisposer()
		ivuNode._inner.disposer.childrenInterceptDisposer && ivuNode._inner.disposer.childrenInterceptDisposer()
	}
...
function createVueNode(xbsjSceneNode) {
	...
	if (!!xbsjSceneNode.children) {
		const childrenInterceptDisposer = XE.MVVM.reaction.intercept(xbsjSceneNode.children, changed => {
			if (changed.type === 'splice') {
				// changed.index;
				// changed.removedCount;
				// changed.added;
				const added = changed.added.map(e => createVueNode(e))
				const toDels = ivuNode.children.slice(changed.index, changed.index + changed.removedCount)
		...
		childrenInterceptDisposer && (ivuNode._inner.disposer.childrenInterceptDisposer = childrenInterceptDisposer)
	}
	...
	methods: {
		...
		popContextMenu({ item, vueObject }) {
					...
					func: () => {
						...
						if (this._currentSceneNode && sn !== this._currentSceneNode) {
							if (sn instanceof XE.SceneTree.Group) {
								...
								sn.parent && this._currentSceneNode.moveTo(sn.parent, sn.parent.children.indexOf(sn))
							}
						}
			...
			if (item.type == 'Tileset') {
				baseItems.splice(1, 0, {
					text: this.lang.restore,
					keys: '',
					func: () => {
						this.$root._colorConfig.forEach(f => f.tileset._tileset.makeStyleDirty())
			...
			if (item.children) {
				...
				if (item._inner.sn.czmObject.cameraAttached !== undefined) {
					baseItems.unshift({
						...
						func: () => {
							item._inner.sn.czmObject.cameraAttached = !item._inner.sn.czmObject.cameraAttached
						}
					})
				}
				baseItems.unshift({
					...
					baseItems.push(
						...[
							...
							},
							{
								...
								func: () => {
									this.$root.$earthUI.showPropertyWindow(item._inner.sn.czmObject, {
							...
							},
							{
								...
								func: () => {
									//显示移动编辑面板
									item._inner.sn.czmObject.positionEditing = !item._inner.sn.czmObject.positionEditing
		...
		itemDoubleClick({ item, vueObject }) {
			...
			if (czmObject) {
				...
				//根据类型去显示界面
				if (t == 'XbsjTileset') this.$root.$earthUI.controls.mainBar.showPage('model')
				else if (t == 'XbsjImagery') this.$root.$earthUI.controls.mainBar.showPage('imagery')
				else if (t == 'XbsjTerrain') this.$root.$earthUI.controls.mainBar.showPage('terrain')
			}
		},
		...
		canMoveItem({ item, vueObject }) {
			...
			const sn = item._inner.sn
			if (this._currentSceneNode && sn !== this._currentSceneNode) {
				if (sn instanceof XE.SceneTree.Group || !this._currentSceneNode.canMoveTo(sn.parent)) {
