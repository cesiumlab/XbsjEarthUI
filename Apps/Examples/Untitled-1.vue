...
                            </div>
                            <div><span style="vertical-align: middle;">采样间距:</span>
                                <input :class="!computing ? '': 'notInput'" v-model.number="gridWidth" class="gridWidth"> m
                                <br/>
                                <span style="vertical-align: middle;">基准面高程:</span>
                                <input :class="!computing ? '': 'notInput'" v-model.number="height" class="gridWidth"> m
                                <button class="xbsj-button" :disabled="computing" @click="startClick">开始分析</button></div>
                        </div>
                    </div>
                </div>
            `,
  ...
  mounted() {
    // 1.1.1 创建地球
    var earth = new XE.Earth(this.$refs.earthContainer); // 1.1.2 场景配置
    ...
    var tileset2 = earth.sceneTree.root.children[1].czmObject; // only for Debug

    window.earth = earth;
    window.tileset1 = tileset1;
    window.tileset2 = tileset2; // 1.1.5 数据绑定 // disposers 用来收集资源销毁函数，并在析构时自动调用！
  ...
  methods: {
    ...
    buttonClick() {
      ...
      this._cutFillComputing.positions = []; //下一个帧再次开始
      this.$nextTick(() => {
  ...
  watch: {
    show(v) {
      if (v) {
        this._cutFillComputing.polygonCreating = true; // this._cutFillComputing.showHelper = true;
      } else {
        this._cutFillComputing.clearResults();
        this._cutFillComputing.polygonCreating = false;
        this._cutFillComputing.positions = [];
...
}; // 2 创建vue程序 // XE.ready()用来加载Cesium.js等相关资源
